    ...
    if len(request.user.groups.all()) > 0:
        if request.user.groups.all()[0].name == '省院':
            for site in WebSite.objects.all():
                ...
                site_data['regdate'] = site.regdate
...
def get_site_count(site_name):
    ...
    time_query = 'processed_at:[' + datetime.strftime(yesterday, '%Y-%m-%d 00:00:00') + ' TO *]'
    query[fq] = time_query
    ...
    time_query = 'processed_at:[' + datetime.strftime(this_month, '%Y-%m-%d 00:00:00') + ' TO *]'
    query[fq] = time_query
    month_count = get_solr_count(query)

    time_query = 'processed_at:[' + this_year + '-01-01 00:00:00 TO *]'
    query[fq] = time_query
...
def get_solr_count(query):
    query_response = Solr(SOLR_SERVER).search(**query)
    count = len(query_response.documents)
    return count
